...
int serialWrapperFTDI::readData(unsigned int arg_bytesToRead, unsigned int& arg_bytesRead, unsigned char* arg_buffer) {
    ...
    FT_STATUS lcl_status = FT_Read(m_ftHandle, arg_buffer, arg_bytesToRead, &arg_bytesRead);
    if (lcl_status != FT_OK) {
